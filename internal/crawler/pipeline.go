...
type Middleware func(doc *models.Document) (bool, error)
...
func (p *Pipeline) Process(doc *models.Document) (bool, error) {
...
func (p *Pipeline) ProcessSync(doc *models.Document) (bool, error) {
	return p.Process(doc)
}

// Built-in middleware functions

// FilterByDocumentType creates a middleware that filters documents by type.
// If the document type doesn't match, it will be skipped.
func FilterByDocumentType(allowedTypes ...models.DocumentType) Middleware {
	allowedSet := make(map[models.DocumentType]bool)
	for _, dt := range allowedTypes {
		allowedSet[dt] = true
	}

	return func(doc *models.Document) (bool, error) {
		if !allowedSet[doc.Type] {
			return false, nil
		}
		return true, nil
	}
